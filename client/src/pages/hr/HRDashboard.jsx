...
const HRDashboard = () => {
    const { user } = useAuth();
    const [stats, setStats] = useState({ jobs: 0, total: 0, shortlisted: 0, interviews: 0 });
    const [recentApps, setRecentApps] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        Promise.all([applicationService.getAllApplications(), jobService.getHRJobs()])
            .then(([appsRes, jobsRes]) => {
                ...
                setStats({
                    ...
                    total: apps.length,
                    // Backend status enum uses capitalised strings
        ...
        <div className="animate-fade-in">
            <div className="mb-6">
                ...
                <p className="page-subtitle">
                    Welcome back, {user?.name?.split(' ')[0]}. Here's your hiring overview.
                </p>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {statCards.map(({ icon: Icon, label, value, color, bg, to }) => (
                    <Link key={label} to={to} className="stat-card hover:shadow-md transition-all">
