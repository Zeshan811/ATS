...
const ApplyJobPage = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            // Backend expects fields named "resume" and "coverLetter"
            formData.append('resume', resumeFile);
            formData.append('coverLetter', coverFile);

            // POST /api/applications/:jobId
            await applicationService.apply(jobId, formData);
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="card p-5">
                    <h2 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                        <FileText className="w-4 h-4 text-primary-600" /> Resume (Required, PDF)
                    </h2>
                    <label
                    ...
                    >
                        ...
                        {resumeFile ? (
                            <div className="text-center">
                                ...
                                <p className="text-sm font-semibold text-primary-700">{resumeFile.name}</p>
                                <p className="text-xs text-primary-500">{(resumeFile.size / 1024).toFixed(1)} KB</p>
                            </div>
                        ) : (
                            <div className="text-center">
                ...
                <div className="card p-5">
                    <h2 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                        <Upload className="w-4 h-4 text-primary-600" /> Cover Letter (Required, PDF or DOCX)
                    </h2>
                    <label
                    ...
                    >
                        ...
                        />
                        {coverFile ? (
                            <p className="text-sm font-semibold text-green-700">{coverFile.name}</p>
                        ) : (
                            <div className="text-center">
                ...
                <div className="flex gap-3">
                    <Link to={`/jobs/${jobId}`} className="btn-secondary flex-1 justify-center">
                        Cancel
                    </Link>
                    <button
                        type="submit"
